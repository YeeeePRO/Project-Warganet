...
const LOCATION_GUIDE = {
    balai_kota: { name: "Balai Kota WARGANET", area: "Verdant Bluffs", desc: "tempat membuat KTP dan daftar pekerjaan", commands: ["/buatktp", "/daftarpekerjaan"] },
    starterpack: { name: "Starterpack", area: "Bandara Internasional WARGANET", desc: "di depan pintu masuk bandara", command: "/claimsp" },
    rumah_sakit: { name: "Rumah Sakit WARGANET", area: "Jefferson", desc: "tempat berobat dan respawn", commands: ["/respawn", "/obat"] },
    polisi: { name: "Markas Polisi WARGANET", area: "Verdant Bluffs", desc: "tempat melapor dan buat SKCK", commands: ["/lapor", "/skck"] },
    bandara: { name: "Bandara Internasional WARGANET", area: "Bandara", desc: "tempat kedatangan dan keberangkatan" },
...
- Server: WARGANET Indonesia
- Kota IC: WARGANET (bukan Los Santos)
- Region: San Andreas

1. APA ITU ROLEPLAY?
Roleplay adalah bermain peran sebagai karakter fiksi di kota WARGANET.
...
app.post('/api/npc/chat', async (req, res) => {
    try {
        ...
        let jobInfo = "";
        if (isJobQuestion) {
            jobInfo = `Pekerjaan di WARGANET: ${jobListShort}. Saran pemula: Petani, Nelayan (Carnival), Kuli Angkut.`;
        }

        let factionInfo = "";
        if (isFactionQuestion) {
            factionInfo = `Faksi di WARGANET: ${factionListShort}`;
        }

        const systemPrompt = `
Kamu WARGANET Guardian, petugas bandara di WARGANET. Kamu dibuat developer Chess7.
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('====================================');
    console.log('[NPC AI] WARGANET Guardian READY');
    console.log(`Running on port ${PORT}`);
    console.log(`Server: WARGANET Indonesia | Developer: Chess7`);
    console.log('====================================');
});